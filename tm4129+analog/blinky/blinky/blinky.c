...
uint8_t Array_Count;
uint8_t DrawLCD,DrawPoint,SweepLinear=2;
uint8_t DC_Count;
DC Voltage[210];
...
ConfigureUART(void)
{
    ...
    //
    UARTStdioConfig(0, 115200, 1600000);
...
//                        AMP_Temp = pow(ADC_Value1,2.0)+pow(ADC_Value2,2.0);
                ...
                if(SweepLinear==0||SweepLinear==1)
                {
                    if(SweepLinear==0)
                    {
                        FreValue = FreValue+10;
                        if(FreValue > 4000000)
                        SysCtlDelay(50000);
                        Write_Quadrature(FreValue);
                    }
                    else if(SweepLinear==1)
                    {
                        FreValue = FreValue+1000;
                        if(FreValue > 4000000)
                            FreValue = 10000;
                        SysCtlDelay(50000);
                        Write_Quadrature(FreValue);                            
